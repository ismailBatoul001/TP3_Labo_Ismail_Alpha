...
BEGIN
  ...
  id_chercheur    NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  nom             VARCHAR2(100) NOT NULL,
  prenom          VARCHAR2(100) NOT NULL,
  specialite      VARCHAR2(50) NOT NULL,
  date_embauche   DATE NOT NULL,

  CONSTRAINT ck_chercheur_date_embauche CHECK (date_embauche <= SYSDATE),
...
BEGIN
  EXECUTE IMMEDIATE '
    CREATE TABLE PROJET (
  id_projet  NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  ...
  CONSTRAINT ck_projet_domaine CHECK (domaine IS NOT NULL),
...
BEGIN
  ...
  CONSTRAINT ck_equipement_categorie CHECK (categorie IS NOT NULL),
  CONSTRAINT ck_equipement_etat CHECK (etat IN (''Disponible'', ''Enmaintenance'', ''Hors service'')),
  CONSTRAINT ck_equipement_date CHECK (date_acquisition <= SYSDATE)
)';
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  ...
  id_affect         NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
BEGIN
  ...
  id_exp            NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  id_projet         NUMBER NOT NULL,
...
BEGIN
  ...
  id_echantillon      NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
