...
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        ROLLBACK;
        RAISE_APPLICATION_ERROR(-20002, 'id_projet déjà existant');
    WHEN OTHERS THEN
        ROLLBACK;
...
BEGIN
    ...
    IF v_disponible = 0 THEN
        RAISE_APPLICATION_ERROR(-20010, 'Équipement non disponible');
    END IF;
...
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        ROLLBACK;
        RAISE_APPLICATION_ERROR(-20011, 'id_affect déjà existant');
    WHEN OTHERS THEN
        ROLLBACK;
